...
import { CreateSalePaymentData } from "@/types";

interface PaymentManagerProps {
  totalAmount: number;
  advances: CreateSalePaymentData[];
  onAdvancesChange: (advances: CreateSalePaymentData[]) => void;
  installmentConfig: {
    remainingAmount: number;
    numberOfInstallments: number;
    startDate: Date;
  } | null | undefined;
  onInstallmentConfigChange: (config: {
    remainingAmount: number;
    numberOfInstallments: number;
    startDate: Date;
  } | null | undefined) => void;
...
export default function PaymentManager({
  ...
  const addAdvance = () => {
    if (newAdvance.amount && newAdvance.dueDate) {
      ...
      if (amount > 0 && amount <= remainingAmount) {
        const advance: CreateSalePaymentData = {
          type: 'ADVANCE',
          amount,
          dueDate: new Date(newAdvance.dueDate),
        };
        onAdvancesChange([...advances, advance]);
  ...
  const clearInstallmentConfig = () => {
    onInstallmentConfigChange(null);
